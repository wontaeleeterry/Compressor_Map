...
Pressure_r = np.round(np.array(Pressure_r, dtype=np.float64), 2)
Flow_r = np.round(np.array(Flow_r, dtype=np.float64), 2)
Effi = np.round(np.array(Effi, dtype=np.float64), 2)
...
# meshgrid 변경: 기존 맵 대비 차이점 확인 (240613)
import matplotlib.tri as tri

# Create grid value first
ngridx = 52
ngridy = 52
xi = np.linspace(0, 0.4, ngridx)
yi = np.linspace(0, 4.5, ngridy)
...
Xi = np.round(Xi, 3)
Yi = np.round(Yi, 3)   # 소수점 셋째자리까지 설정 - plot 해상도에 영향

#ax.contourf(Xi, Yi, Zi, levels = 30, cmap = 'jet')
CS = plt.contourf(Xi, Yi, Zi, levels = 128, cmap = 'jet')
...
#CS_1 = plt.contourf(Xi, Yi, Zi, levels = np.arange(0.3, 0.9, 0.1), cmap = 'black')
